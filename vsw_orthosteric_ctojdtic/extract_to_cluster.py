#########How to use?
######### $SCHRODINGER/run ./extract_to_cluster.py for_next_phase_only_lig_out_4removed_fixed_toextract.maegz
      ...
      with structure.StructureReader(filename) as reader:
           for i, st in enumerate(reader):
              # print(st.property.keys())
               inf = [i, st.title, st.property['i_user_Extract']]
               get_list.append(inf)
      return get_list
...
if __name__ == "__main__":
      ...
      with structure.StructureWriter("to_cluster_binding_domain.maegz") as writer:
